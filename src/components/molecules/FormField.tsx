...
export const FormField: React.FC<FormFieldProps> = ({ label, children }) => {
  return (
    <div className="relative mt-2">
      {children}
      <label className="absolute left-2 -top-2.5 px-1 bg-white text-[12px] text-[#4BB8FA] font-medium transition-all duration-200 
        pointer-events-none
        ...
        peer-focus:-top-2.5 peer-focus:left-2 peer-focus:text-[12px] peer-focus:text-[#4BB8FA] peer-focus:bg-white peer-focus:font-medium">
        {label}
      </label>
    </div>
  );
 ...
 */
export const FORM_INPUT_CLASSES = "peer w-full border-2 border-gray-200 rounded p-3.5 text-[#2C5EAD] outline-none bg-transparent focus:border-[#4BB8FA] transition-all";